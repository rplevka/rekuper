...
def parse_jenkins_job(correlation_id):
    ...
    job_param_actions = [j for j in job_response.json()['actions'] if j.get('_class') == 'hudson.model.ParametersAction']
    try:
        job_params = job_param_actions[0]['parameters']
        umb_message_params = [i['value'] for i in job_params if i['name'] == 'CI_MESSAGE']
    except IndexError():
    ...
    try:
        ...
        log.error(f'error: no CI_MESSAGE parameters found for the given job: {jenkins_url}')
        raise
...
for resource in ['instances', 'containers']:
    ...
    while range_start <= timestamp_now:
        ...
        if query_request.json()['data']['result']:
            ...
            timestamps = [
                (i['values'][0][0], i['values'][-1][0]) for i in query_request.json()['data']['result']
            ]
